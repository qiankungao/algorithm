...
func expandCluster(point Point, neighborPoints []Point, points []Point, currentClusterN int, epsilon float64, minPoints int) []Point {
	...
	for i, _ := range neighborPoints {
		if !neighborPoints[i].visited {
			...
			newPoints = regionQuery(neighborPoints[i], points, epsilon)
...
func main() {
	...
	point1 := model.Point{Entry: []float64{0, 0, 0}}
	//	fmt.Println(point1)
	point2 := model.Point{Entry: []float64{1, 1, 10}}
	point3 := model.Point{Entry: []float64{1, 2, 10}}
	point4 := model.Point{Entry: []float64{1, 3, 10}}
	point5 := model.Point{Entry: []float64{1, 4, 10}}

	one := make([]model.Point, 0)
	one = append(one, point1)
	one = append(one, point2)
	one = append(one, point3)

	other := make([]model.Point, 0)
	other = append(other, point4)
	other = append(other, point5)

	mer := common.Merge(one, other)
	fmt.Println("mer:", mer)
